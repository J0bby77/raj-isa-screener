#!/usr/bin/env python3
"""
score_partab.py  --  ISA Part A/B Formatter, Conviction Scorer & Email Prep
...
Supports three source pipelines — routing is driven by each entry's "_source_pipeline" field:
  ...
  "growth_stock"  — standard 14-metric Part A (max 28) + 13-metric Part B (max 26) = /54
...
PART_B_STRONG_THRESHOLD   = 19    # /26
PART_B_ACCEPTABLE_MIN     = 11
HIGH_SCORE_THRESHOLD      = 40    # total /54 — analyst disparity trigger

# Analyst rating values that do NOT trigger disparity flag
STRONG_RATINGS = {"strongbuy", "strong buy", "buy"}

# Growth stock conviction brackets (/54 → preliminary bracket for email)
...
ENERGY_HIGH_SCORE         = 28    # /36 (~78%, proportional to growth 40/54) — disparity trigger
...
# ---------------------------------------------------------------------------
def prelim_conviction_bracket(total_score: int | None) -> dict:
    """Growth stock preliminary conviction bracket (/54)."""
    ts = total_score or 0
    for threshold, label, level, note in SCORE_TO_PRELIM_CONVICTION:
        if ts >= threshold:
            return {
                "total_score_54":   ts,
                "bracket":          label,
    ...
    return {"total_score_54": ts, "bracket": "No Action", "level": "low",
            "conviction_score": "[Claude fills /100 at Step 9]"}
...
def get_conviction_bracket(s: dict) -> dict:
    ...
    if pipeline == "energy":
        ...
        return prelim_conviction_bracket(s.get("total_score"))
...
def build_s5_row(ticker: str, s: dict, wl_entry: dict | None) -> dict:
    ...
    to the source_pipeline: growth_stock (/54), energy (/36), or vci (ACS/100).
    """
    ...
    if pipeline == "energy":
        ...
        score_display = (
            ...
            f"({s.get('total_score', '?')}/54) | Conv: {conv['conviction_score']}"
        )
...
def build_s7_row(ticker: str, s: dict) -> dict:
    ...
    if pipeline == "energy":
        ...
        score_summary = (f"{s.get('part_a_score','?')}/28 + {s.get('part_b_score','?')}/26 "
                         f"= {s.get('total_score','?')}/54")
...
def _build_s3_skeleton_growth(ticker: str, s: dict) -> dict:
    """Growth stock s3 investment case skeleton (/54 scorecard)."""
    ...
    header_metrics = [
        ...
        {"label": "Part B Score",       "value": f"{s.get('part_b_score','?')}/26",  "assessment": s.get("part_b_status",""),  "signal": "green" if (s.get("part_b_score") or 0) >= PART_B_STRONG_THRESHOLD else "amber"},
        {"label": "Combined Score",     "value": f"{s.get('total_score','?')}/54",   "assessment": conv["bracket"],             "signal": conv["level"] if conv["level"] != "low" else "amber"},
...
def run(metrics_path: str, out_path: str) -> dict:
    ...
    for ticker, s in tickers_data.items():
        ...
        if pipeline == "energy":
            # Energy: disparity triggers at >=28/36 (~78%, proportional to growth 40/54)
    ...
    for ticker, s in tickers_data.items():
        ...
        if pipeline == "energy":
            ...
            # growth_stock
            base.update({
                "total_score_54": s.get("total_score"),
                "part_a_score":   s.get("part_a_score"),
                "part_b_score":   s.get("part_b_score"),
                "score_display":  f"{s.get('total_score', '?')}/54",
                "conviction_score": "[Claude fills /100 at Step 9]",
                "target_upside":  fmt_pct(s.get("target_upside")),
                "action": (
                    "In range — score at Step 9" if s.get("_in_window")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Format Part A/B (growth stock /54), Energy (/36), and VCI (ACS /100) scores "

...
def main():
    ...
    if not ok:
        print("ERROR: Non-ASCII characters detected in email body. Fix before sending.")
        sys.exit(1)
    ...
    char_count = len(body)
    strong_buys_count = len([r for r in full_data if is_strong_buy(r)])
    print(f"Email body written: {args.output}")
    print(f"  Body length: {char_count:,} characters")
    print(f"  Strong Buys: {strong_buys_count}")
    ...
    print(f"  subject:         ISA Growth Stock Analysis -- {args.group} | {args.run_date} | {strong_buys_count} Strong Buys (RETRO SAVED)")
    print(f"  body:            <contents of {args.output}>")
    print(f"  is_html:         true   <-- MANDATORY: MUST always be present")
    print("=" * 60)
